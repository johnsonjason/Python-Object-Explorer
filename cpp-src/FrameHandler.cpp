...
__declspec(dllexport) PyFrameObject* GetFrame(PyThreadState* tstate, PyCodeObject* code, PyObject* globals, PyObject* locals)
{
    ...
    if (_Executable) {
        ...
        if (_FrameDebug) {
            ...
            //
            // Do set _Executable to tree so it can do a repeated look-up if the previous case failed, until it hits. Though, the remote socket will generally time-out before then.
            //
            _Executable = true;
        }
        else {
